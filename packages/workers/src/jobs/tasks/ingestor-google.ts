...
export async function ingestorGoogle(date: Date){
  const day = ymd(date);
  const platform: Row['platform'] = 'google';
  ...
  for (const r of rows) {
    params.push(
      r.platform, r.date, r.account_id, r.campaign_id, r.adgroup_id, r.ad_id,
      r.impressions, r.clicks, r.spend, r.conversions, r.revenue, JSON.stringify(r.raw)
    );
  }
